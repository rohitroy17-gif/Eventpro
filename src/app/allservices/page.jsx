...
export default function AllServices() {
  ...
  useEffect(() => {
    if (!apiUrl) return; // make sure apiUrl is defined
    ...
    <div className="p-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {services.map(service => (
          <div key={service._id} className="shadow-lg rounded-xl bg-white overflow-hidden border hover:shadow-xl transition">
            <img src={service.img} alt={service.title} className="w-full h-48 object-cover" />
            <div className="p-4">
              <h3 className="text-xl font-semibold">{service.title}</h3>
              <p className="text-sm text-blue-600 mt-1 font-medium">{service.type}</p>
              <p className="text-gray-500 text-sm mt-1">Date: {new Date(service.date).toLocaleDateString()}</p>
